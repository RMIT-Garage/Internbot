...
import type { TicketListItemResponse, TicketResponse } from '../hooks/useTickets'
...
const STATUS: Record<TicketListItemResponse['status'], { label: string; icon: React.ReactNode }> = {
  ...
  in_progress: { label: 'In Progress', icon: <Clock className="size-3 text-zinc-500" /> },
  resolved: { label: 'Resolved', icon: <CheckCircle className="size-3 text-zinc-900" /> },
...
function TicketRow({
  ...
  onExpand: (id: string) => Promise<TicketResponse | null>
}) {
  ...
  const toggle = async () => {
    if (!open && !detail) {
      ...
      setDetail(await onExpand(ticket.id))
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-zinc-200 bg-white transition hover:border-zinc-300">
      <button onClick={toggle} className="flex w-full items-start gap-3 px-4 py-3.5 text-left">
        <span className="mt-0.5 shrink-0">{status.icon}</span>
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-medium text-zinc-800">{ticket.subject}</p>
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs text-zinc-200">·</span>
            <span className="text-xs text-zinc-400">{timeAgo(ticket.createdAt)}</span>
          </div>
        </div>
      ...
      {open && (
        <div className="border-t border-zinc-100 px-4 py-3">
          {loadingDetail ? (
            ...
            <p className="text-sm whitespace-pre-line text-zinc-500">{detail.body}</p>
          ) : (
            <p className="text-xs text-zinc-400">Could not load ticket details.</p>
          )}
        </div>
      )}
    </div>
...
export function TicketList({ refreshTrigger }: { refreshTrigger?: number }) {
  const { tickets, loading, error, refresh, fetchTicketDetail } = useTickets()
  ...
  return (
    <div>
      ...
      {!loading && tickets.length > 0 && (
        <div className="space-y-2">
          {tickets.map((t) => (
            <TicketRow key={t.id} ticket={t} onExpand={fetchTicketDetail} />
          ))}
        </div>
      )}
    </div>
