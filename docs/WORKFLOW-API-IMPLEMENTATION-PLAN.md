...
- Refactored to backend-mediated **upload intent + storage-event finalize** flow: client `POST`s `/attachments/upload-intents` to get a V4 signed `PUT` URL; backend pre-writes the attachment subdoc with `uploadStatus: 'uploading'`; GCS `OBJECT_FINALIZE` event drives `FinalizeStorageAttachmentCommandHandler` which flips the row to `'finalized'` and records `storageGeneration`. Reads 404 `uploading` rows; offer submission gates on `attachments.some((a) => a.isFinalized())`. Firebase Storage rules removed for attachment paths (kept only for avatars); `firebase_rules_firestore_cross_service` IAM scoped to avatars only. Path parser uses the `att_*` filename prefix to recover the attachment id from `OBJECT_FINALIZE` events. `Attachment` VO gained `uploadStatus` + `isFinalized()` + immutable `withFinalized()`; aggregates gained `finalizeAttachment()` (idempotent under Pub/Sub redelivery). Seed script updated to PUT via signed URL then poll `GET` until `uploadStatus === 'finalized'`.
