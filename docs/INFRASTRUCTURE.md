...
| `terraform-apply-dev.yml`  | Push to develop touches `infrastructure/**` | Apply to dev                                                           |
| `terraform-apply-prod.yml` | Push to main touches `infrastructure/**`    | Apply to prod                                                          |
| `_terraform.yml`           | `workflow_call`                             | Reusable engine — init, fmt, validate, plan/apply                      |

Apply requires OIDC auth via the env's `terraform-ci` SA. The SA has enough IAM scope to manage project resources but no deploy-app permissions.
