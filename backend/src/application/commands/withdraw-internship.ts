...
export interface WithdrawInternshipCommand {
  actor: RequestActor
  internshipId: string
  metadata?: CommandMetadata
}

export interface WithdrawInternshipResult {
  id: string
}

export class WithdrawInternshipCommandHandler {
  constructor(
  ...
  async handle(cmd: WithdrawInternshipCommand): Promise<WithdrawInternshipResult> {
    ...
    return this.uow.execute(async (ctx) => {
      ...
      internship.withdraw(this.idGenerator.next(), new Date())
